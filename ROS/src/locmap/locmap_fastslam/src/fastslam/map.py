...
class Map:
    def __init__(self, amount, expected_nr_of_landmarks=1000):
        """
    ...
    def get_kdtree(self, map_id):
        return self.kdtree[map_id]
    ...
    def add_landmark(self, map_id, landmark, landmark_class=0, score=1):
        """
        ...
        if self.__len__(map_id) == self.supported_nr_of_landmarks:
            self.expand_map()

        # Add it to the map
        offset = self.__len__(map_id)
    ...
    def remove_landmark(self, map_id, offset: float):
        """
        ...
        self.landmarks[
            map_id, :, 3 * offset : 3 * (self.__len__(map_id) - 1)
        ] = self.landmarks[map_id, :, 3 * (offset + 1) : 3 * self.__len__(map_id)]

        self.scores[map_id, offset : self.__len__(map_id) - 1] = self.scores[
            map_id, offset + 1, self.__len__(map_id)
        ]
        self.landmark_classes[
            map_id, offset : self.__len__(map_id) - 1
        ] = self.landmark_classes[map_id, offset + 1, self.__len__(map_id)]
        self.sizes[map_id] -= 1
    ...
    def get_landmarks(self, map_id):
        return self.landmarks[map_id, :, : 3 * self.__len__(map_id)]

    def set_landmark_score(self, map_id, i, score):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        self.scores[map_id, i] = score

    def get_landmark_classes(self, map_id):
        return self.landmark_classes[map_id, : self.__len__(map_id)]

    def get_landmark_class(self, map_id, i):
        return self.landmark_classes[map_id, i]

    def get_landmark_score(self, map_id, i):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        return self.scores[map_id, i]

    def increment_landmark_score(self, map_id, i, amount=1):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        if self.scores[map_id][i] >= 5 and amount > 0:
            return
        self.scores[map_id][i] += amount

    def get_landmark(self, map_id, i):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        return self.landmarks[map_id][:, (3 * i) : (3 * i + 3)]

    def get_landmark_covariance(self, map_id, i):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        return self.landmarks[map_id][:, (3 * i + 1) : (3 * i + 3)]

    def get_landmark_mean(self, map_id, i):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        return self.landmarks[map_id][:, (3 * i)]

    def set_landmark_mean(self, map_id, i, mean):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        self.landmarks[map_id][:, (3 * i)] = mean

    def set_landmark_covariance(self, map_id, i, covariance):
        if i >= self.__len__(map_id):
            raise Exception(f"Index {i} out of range")
        self.landmarks[map_id][:, (3 * i + 1) : (3 * i + 3)] = covariance

    def __len__(self, map_id):
        """
        Returns the number of landmarks in this map
        """
        return int(self.sizes[map_id])

    def search_corresponding_landmark_index(self, map_id, candidate_landmarks_mu):
        """
        ...
        if self.__len__(map_id) == 0:
            return [], []

        if not self.kdtree[map_id]:
            landmark_mus = (
                self.landmarks[map_id][:, 0 : 3 * self.__len__(map_id) : 3]
            ).T
            self.kdtree[map_id] = scipy.spatial.KDTree(landmark_mus)

        distance, index = self.kdtree[map_id].query(candidate_landmarks_mu)
