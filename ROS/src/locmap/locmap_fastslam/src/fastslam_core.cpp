...
vector<int> find2(vector<float> &dx, vector<float> &dy, float phi, float rmax)
{
    ...
    {
        if ((abs(dx[j]) < rmax) && (abs(dy[j]) < rmax) && ((dx[j] * cos(phi) + dy[j] * sin(phi)) > 0.0) && (((float)pow(dx[j], 2) + (float)pow(dy[j], 2)) < (float)pow(rmax, 2)))
        {
            index.push_back(j);
        }
    }
...
vector<LandmarkMetadata> &Particle::metadata()
{
    return _metadata;
}
...
void Particle::setMetadata(vector<LandmarkMetadata> &meta)
{
    _metadata = meta;
}
