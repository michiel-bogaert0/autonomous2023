...
class Rrt:
    ...
    def __init__(self, max_dist: float = 0.5, plan_dist: float = 12.0, max_iter: int = 200, max_angle_change: float = 0.8, safety_dist: float = 0.3, track_width: float = 3, max_track_width: float = 4, search_rad: float = None):
        ...
        self.max_track_width_squared_DEFAULT = max_track_width ** 2         # Maximum width of track
    ...
    def renew(self, cones: np.ndarray, root: Tuple[float, float] = None):
        ...
        if root is not None:
            self.root = RrtNode(0, None, [])
            self.points = np.zeros((1, Rrt.POINT_LEN))
            self.points[self.root.index, [Rrt.IDX_X, Rrt.IDX_Y]] = np.array([root[0], root[1]])
            self.nodes = [self.root]
    ...
    def add_node(self, x: float, y: float):
        ...
        """
        # Find closest node
        diff = (self.points[:, Rrt.IDX_X]-x) ** 2 + (self.points[:, Rrt.IDX_Y]-y) ** 2
        ...
        # Connect new node
        idx_new_node, new_node = self.connect_node(new_x, new_y, best_node, min_cost, cos_angle, mid_cost)

        # Reconnect neighbors for optimal path
        for i in neighbor_idxs:
            if i == idx_best or i == self.root.index:
                continue
            if dists[i] + min_cost < self.points[i, Rrt.IDX_PAD2ROOT] \
                    and not self.check_angle(self.points[i, Rrt.IDX_X], self.points[i, Rrt.IDX_Y], new_node) \
                    and not self.check_safety_distance_cutting(self.points[i, Rrt.IDX_X],
                                                               self.points[i, Rrt.IDX_Y], new_node):
    ...
    def connect_node(self, new_x: float, new_y: float, parent: "RrtNode",
                     pad2root: float, cos_angle: float, mid_cost: float) -> Tuple[int, "RrtNode"]:
        """
        ...
        self.points = np.append(self.points, newpoint.reshape((1, Rrt.POINT_LEN)), axis=0)
    ...
    def find_neighbor_idxs(self, x: float, y: float) -> Tuple[np.ndarray, np.ndarray]:
        ...
        dists = (self.points[:, Rrt.IDX_X]-x) ** 2 + (self.points[:, Rrt.IDX_Y]-y) ** 2
    ...
    def calculate_cost(self, x: float, y: float, parent: "RrtNode", *args, **kwargs):
        ...
        min_b = np.min(diff_b)
        min_y = np.min(diff_y)
        return self.points[parent.index, Rrt.IDX_MID_COST] + (min_b - min_y) ** 2, min_b, min_y

    def rescale(self, point: np.ndarray, x: float, y: float, max_distance: float) -> Tuple[float, float]:
        """
        ...
        if dist <= max_distance and False:
            ...
            return point[Rrt.IDX_X] + vec_x * max_distance/dist, point[Rrt.IDX_Y] + vec_y * max_distance/dist

    def get_cost_branch(self, branch: np.ndarray, cones: np.ndarray) -> Tuple[float, float, float]:
        ...
        angles[1:] = np.arctan2(branch[1:, self.IDX_Y] - branch[:-1, self.IDX_Y],
        ...
        for idx, point in enumerate(branch):
            ...
            trans = np.array([
                [1, 0, -point[self.IDX_X]],
                [0, 1, -point[self.IDX_Y]],
                [0, 0, 1]
            ])
        ...
        color_cost = ((np.count_nonzero(np.array(global_right_cones_array)[:, -1] == 0) + \
                       np.count_nonzero(np.array(global_left_cones_array)[:, -1] == 1)) / len(cones)) ** 2
    ...
    def calculate_path(self) -> Tuple[float, np.ndarray]:
        ...
        leaf_cost = ((leaves[:, Rrt.IDX_DIST2ROOT] - self.plan_dist) / self.plan_dist) ** 2
        # Average angle change
        angle_cost = (leaves[:, Rrt.IDX_ANGLE] / leaves[:, Rrt.IDX_HOPS2ROOT] / self.max_angle_change) ** 2
        # Middle cost
        ...
        if(len(tot_cost) == 0):
            parent = Node(0,
                          ...
                          0,
                          0)
            return 0, np.array([parent,
                                Node(self.max_dist/2,
                                     0,
                                     0, parent, [],
                                     0,
                                     0)])
        ...
        while node is not None:
            ...
            path = np.append([Node(self.points[idx, Rrt.IDX_X],
                                   self.points[idx, Rrt.IDX_Y],
                                   dist, node, [],
                                   angle,
                                   angle_change)], path, axis=0)
    ...
    def calc_cos_angle(self, x: float, y: float, node: "RrtNode") -> float:
        ...
        vector_a = np.array([node_point[Rrt.IDX_X] - parent_point[Rrt.IDX_X],
    ...
    def check_angle(self, x: float = None, y: float = None, parent: "RrtNode" = None, cos_angle: float = None) -> bool:
        """
        Checks if cosinus of angle is not too big
        Args:
    ...
    def check_safety_distance(self, x: float, y: float) -> bool:
        ...
        Args:
            x: x-coordinate we want to add
            y: y-coordinate we want to add
        ...
        return np.any(((self.cones[:, 0] - x)**2 + (self.cones[:, 1] - y)**2) <= self.safety_dist_squared)

    def check_safety_distance_cutting(self, x: float, y: float, parent: "RrtNode") -> bool:
        """
        ...
        if len(p) == 0:
            return False
        return np.any(np.sum(np.power(p-self.cones[t_filter][:, [0, 1]], 2), axis=1) < self.safety_dist_squared)

    def update_params(self):
        sum = self.safety_dist_faults + self.angle_faults + self.mid_faults + self.cut_faults
        # TODO: * Don't hardcode threshold
        #       * Increase iteration count when faults exceed threshold,
    ...
    def rdm_new_node(self) -> Tuple[float, float]:
        ...
        d = self.plan_dist
        r = random.random() * d
        # TODO: Don;t hardcode factor 1.5
        theta = (random.random() - 0.5) * 2 * self.max_angle_change*1.5
    ...
    def run(self, front_cones):
        ...
        # TODO: * Make iterations not hardcoded and dynamic (see TODO update_params())!
        for i in range(750):
            done = False
