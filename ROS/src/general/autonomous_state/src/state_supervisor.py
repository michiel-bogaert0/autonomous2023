...
class StateSupervisorNode:
    def __init__(self):
        ...
        self.timeout = rospy.get_param('~timeout', 10.0)  # Default timeout is 10 seconds
    ...
    def run(self):
        ...
        rospy.loginfo(f"Monitoring state topic for {AutonomousStatesEnum.ASFINISHED} with a timeout of {self.timeout} seconds.")
        ...
        while not rospy.is_shutdown():
            if self.state == AutonomousStatesEnum.ASFINISHED:
                ...
                rospy.logerr(f"{AutonomousStatesEnum.ASEMERGENCY} received. Exiting with an error")
                ...
                rospy.logerr(f"Timeout reached. {AutonomousStatesEnum.ASFINISHED} not received. Exiting with an error.")
                raise rospy.ROSException(f"Timeout reached")
