...
class Node:
    def __init__(self, name, pid):
        ...
        self.cpu_publisher = rospy.Publisher(
            ns_join("~", name[1:], "cpu"), Float32, queue_size=20
        )
        self.mem_publisher = rospy.Publisher(
            ns_join("~", name[1:], "mem"), UInt64, queue_size=20
        )
...
if __name__ == "__main__":
    ...
    for mem_topic in mem_topics:
        mem_publishers.append(
            rospy.Publisher("~total_%s_mem" % mem_topic, UInt64, queue_size=20)
        )
    ...
    while not rospy.is_shutdown():
        for node in rosnode.get_node_names():
            ...
            if not node_api:
                rospy.logerr(
                    "[cpu monitor] failed to get api of node %s (%s)" % (node, node_api)
                )
                continue
            ...
            local_node = (
                "localhost" in node_api
                or "127.0.0.1" in node_api
                or (this_ip is not None and this_ip == ros_ip)
                or subprocess.check_output("hostname").decode("utf-8").strip()
                in node_api
            )
            if not local_node:
                ignored_nodes.add(node)
                rospy.loginfo(
                    "[cpu monitor] ignoring node %s with URI %s" % (node, node_api)
                )
                continue
            ...
            try:
                ...
                rospy.logerr(
                    "[cpu monitor] failed to get pid of node %s (api is %s)"
                    % (node, node_api)
                )
            else:
                try:
                    ...
                    rospy.logerr(
                        "[cpu monitor] failed to get pid for node %s from NODEINFO response: %s"
                        % (node, resp)
                    )
                else:
                    try:
                        ...
                        rospy.logwarn(
                            "[cpu monitor] psutil can't see %s (pid = %d). Ignoring"
                            % (node, pid)
                        )
                        ignored_nodes.add(node)
                    else:
        ...
        if save_to_csv:
            ...
            elif (rospy.get_rostime() - node_start_time) > rospy.Duration(
                5
            ):  # wait 5 seconds before setting header
                rospy.loginfo(
                    "[cpu monitor] waited for 5 seconds before initializing csv header"
                )
