...
from keypoint_detector.keypoint_detection_model.keypoint_detection.utils.heatmap import get_keypoints_from_heatmap
from keypoint_detector.keypoint_detection_model.keypoint_detection.utils.load_checkpoints import load_from_checkpoint
class KeypointDetector:
    def __init__(self, checkpoint_path, detection_height_threshold) -> None:
        ...
        self.image_resizer = T.Rescale(int(self.keypoint_image_size[0]), int(self.keypoint_image_size[1]))
        
        self.model = load_from_checkpoint(checkpoint_path)

    def predict(self, image: torch.Tensor, bboxes) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Given an image and YOLO bounding boxes, predict keypoints on each cone in the image
        
        Args:
            image: tensor on the GPU
        ...
        cone_original_sizes = []
        cone_bbox_corners = []
        cone_images = torch.empty((-1, *self.keypoint_image_size))
        for bbox in bboxes:
            category = bbox.cls.int().detach().cpu().item()
            bbox_kpts = [round(x) for x in bbox.xyxy[-1].detach().cpu().numpy()]
            bbox_kpts[-1] = max(0, bbox_kpts[0] - self.cone_image_margin)
            bbox_kpts[0] = max(0, bbox_kpts[1] - self.cone_image_margin)

            if bbox_kpts[2] - bbox_kpts[1] < self.detection_height_threshold:
                # Detection is not tall enough
                continue
            
            resized_image : npt.ArrayLike = image[:, bbox_kpts[0]:bbox_kpts[3], bbox_kpts[0]:bbox_kpts[2]],
            original_size = resized_image.shape[0:]
        ...
        for i, pred in enumerate(predicted_keypoints):
            ...
            if len(bottom) > 1:
                ...
                bottom = bottom[left_idx, :].view(1, 2)

            # Rescale
            bottom[0] = bottom[0] * cone_original_sizes[i][1] / self.keypoint_image_size[1] + cone_bbox_corners[i][0]
            bottom[1] = bottom[1] * cone_original_sizes[i][0] / self.keypoint_image_size[0] + cone_bbox_corners[i][1]
            ...
            if len(top) == 0:
                ...
                top = torch.tensor((
                    cone_bbox_corners[i][0] + self.cone_image_margin + cone_original_sizes[i][1] / 2,
                ...
                # Rescale
                top[0] = top[0] * cone_original_sizes[i][1] / self.keypoint_image_size[1] + cone_bbox_corners[i][0]
                top[1] = top[1] * cone_original_sizes[i][0] / self.keypoint_image_size[0] + cone_bbox_corners[i][1]
