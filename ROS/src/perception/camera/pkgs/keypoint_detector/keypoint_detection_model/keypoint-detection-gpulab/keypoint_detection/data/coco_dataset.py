...
class COCOKeypointsDataset(ImageDataset):
    ...
    def add_argparse_args(
        ...
        parser.add_argument(
            ...
            default=0,
            type=int,
            help="minimal cone height for keypoints to be detected",
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.dataset_dir_path = (
            self.dataset_json_path.parent
        )  # assume paths in JSON are relative to this directory!
    ...
    def prepare_dataset(self):
        ...
        with open(self.dataset_json_path, "r") as file:
            ...
            for annotation in parsed_coco.annotations:
                ...
                if keypoints[4] - keypoints[1] < self.min_cone_height:
                    continue
            ...
            for img_id, keypoint_dict in annotation_dict.items():
                img_channels_keypoints = [
                    [] for _ in range(len(self.keypoint_channel_configuration))
                ]
    ...
    @staticmethod
    def split_list_in_keypoints(
        list_to_split: List[COCO_KEYPOINT_TYPE],
    ) -> List[List[COCO_KEYPOINT_TYPE]]:
        """
    ...
    def collate_fn(data):
        ...
        reordered_keypoints = [
            [keypoints[i][j] for i in range(len(keypoints))]
