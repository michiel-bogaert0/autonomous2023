...
def overlay_image_with_heatmap(
    images: torch.Tensor, heatmaps: torch.Tensor, alpha=0.5
) -> torch.Tensor:
    """ """
...
def overlay_image_with_keypoints(
    images: torch.Tensor, keypoints: List[torch.Tensor], sigma: float
) -> torch.Tensor:
    """
    ...
    for i in range(images.shape[0]):
        heatmaps = generate_channel_heatmap(
            image_size, keypoints[i], sigma=sigma, device="cpu"
        )  # C x H x W
...
def visualize_predictions(
    ...
    predicted_heatmap_overlays = overlay_image_with_heatmap(
        imgs[:num_images], predicted_heatmaps[:num_images]
    )
    gt_heatmap_overlays = overlay_image_with_heatmap(
        imgs[:num_images], gt_heatmaps[:num_images]
    )
    predicted_keypoints = [
        torch.tensor(
            get_keypoints_from_heatmap(
                predicted_heatmaps[i].cpu(), minimal_keypoint_pixel_distance
            )
        )
    ...
    images = torch.cat(
        [predicted_heatmap_overlays, predicted_keypoints_overlays, gt_heatmap_overlays]
    )
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(
        dataset, batch_size, shuffle=False, num_workers=0, collate_fn=dataset.collate_fn
    )
    ...
    heatmaps = create_heatmap_batch(
        shape, keypoint_channels[0], sigma=6.0, device="cpu"
    )
