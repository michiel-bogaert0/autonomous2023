...
class MultiChannelKeypointsCompose(A.Compose):
    ...
    def __init__(self, transforms, p: float = 1):
        keypoint_params = A.KeypointParams(
            format="xy", label_fields=["channel_labels"], remove_invisible=True
        )
    ...
    def __call__(
        ...
        data["channel_labels"] = self.flatten_keypoints(
            self.create_channel_labels(keypoints)
        )
        # apply transforms
    ...
    def order_transformed_keypoints_by_channel(
        ...
        for transformed_keypoint, channel_label in zip(
            transformed_keypoints, transformed_channel_labels
        ):
