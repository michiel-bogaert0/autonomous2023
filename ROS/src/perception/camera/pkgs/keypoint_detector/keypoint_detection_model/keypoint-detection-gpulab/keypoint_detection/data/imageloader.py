        ...
        for j in range(self.n_io_attempts):
            try:
                ...
                if j == self.n_io_attempts - 1:
                    raise IOError(
                        f"Could not load image for dataset entry with path {path}, index {idx}"
                    )

                sleep_time = max(random.gauss(sleep_time_in_seconds, j), 0)
                print(
                    f"caught IOError in {j}th attempt to load image for {path}, sleeping for {sleep_time} seconds"
                )
