...
class KeypointAPMetric(Metric):
    ...
    def __init__(self, keypoint_threshold_distance: float, dist_sync_on_step=False):
        ...
        self.add_state(
            "classified_keypoints", default=default(), dist_reduce_fx="cat"
        )  # list of ClassifiedKeypoints
        self.add_state(
            "total_ground_truth_keypoints",
            default=torch.tensor(0),
            dist_reduce_fx="sum",
        )

    def update(
        self, detected_keypoints: List[DetectedKeypoint], gt_keypoints: List[Keypoint]
    ):
    ...
    def compute(self):
        p, r = calculate_precision_recall(
            self.classified_keypoints, int(self.total_ground_truth_keypoints.cpu())
        )
...
class KeypointAPMetrics(Metric):
    ...
    def __init__(
        self, keypoint_threshold_distances: List[float], dist_sync_on_step=False
    ):
        super().__init__(dist_sync_on_step=dist_sync_on_step)
...
if __name__ == "__main__":
    print(
        check_forward_full_state_property(
            ...
            input_args={
                "detected_keypoints": [DetectedKeypoint(10, 20, 0.02)],
                "gt_keypoints": [Keypoint(10, 23)],
            },
        )
    )
