...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, n_channels_in, n_channels=32):
        ...
        self.conv1 = nn.Conv2d(
            n_channels_in, n_channels, kernel_size=(3, 3), padding=1, bias=False
        )
        self.bn1 = nn.BatchNorm2d(n_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(
            n_channels, n_channels, kernel_size=(3, 3), padding=1, bias=False
        )
...
class S3K(Backbone):
    ...
    def __init__(self, **kwargs):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=3, out_channels=3, kernel_size=self.kernel_size, padding="same"
        )
        self.norm1 = nn.BatchNorm2d(num_features=3)
        self.conv2 = nn.Conv2d(
            in_channels=3, out_channels=16, kernel_size=self.kernel_size, stride=(2, 2)
        )
        self.norm2 = nn.BatchNorm2d(num_features=16)
        self.conv3 = nn.Conv2d(
            in_channels=16, out_channels=32, kernel_size=self.kernel_size, stride=(2, 2)
        )
        ...
        self.up1 = nn.ConvTranspose2d(
            in_channels=32, out_channels=32, kernel_size=self.kernel_size, stride=(2, 2)
        )
