...
/**
    ...
    \brief          Provides a String object to aid the development with
   different compilers

    Unfortunately the C++ string object is not stable, which means the standard
   string object <std::string> cannot be linked if the compiler version does not
   match. To overcome this nuisance this Object provides a string wrapper which
   is compatible with <std::string>
*/
...
public:
  ...
  */
  NeoString(const char *text); // cppcheck-suppress noExplicitConstructor
...
public:
  ...
  */
  const NeoStringListConstIterator &
  operator=(const NeoStringListConstIterator &object);
...
/**
    ...
    This class will help you to iterate through a list of strings, it implements
   iterator functionality and can be used as below.
...
/**
    \ingroup       All
    \ingroup       Exception
    \brief         Requesting an image while holding all available image
   resources
*/
