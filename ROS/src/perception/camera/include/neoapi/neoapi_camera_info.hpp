/* \file neoapi_camera_info.hpp
 ...
 Interface. Copyright (c) by Baumer Optronic GmbH. All rights reserved, please
 see the provided licence document for full details.
 */
...
/**
    ...
    \brief      Camera info container class which offers basic information about
   an available camera

    If neoapi.CamInfoList is called it will return a list of cameras, you can
   use the neoapi.CamInfo() class to get information about a camera in this
   list.
*/
...
public:
  ...
  */
  virtual ~CamInfo();
  /**
      \brief      Get the ID of the camera, this is unique to each camera in
     combination with a Baumer producer \return     The ID
  */
...
/**
    \ingroup    All
    \ingroup    Main
    \brief      Provides a list of physically connected cameras available to be
   used/connected with neoAPI

    You can use this class if you don't know what camera(s) might be connected
   during program operation. With the neoapi::CamInfoList::Refresh() method you
   can update the list to reflect current system status We provide Plug and Play
   Events for the CamInfoList class which you can poll or register an callback
   for. Those will notify you of any changes on the system. If you get such an
   event, you still need to call neoapi. CamInfoList.Refresh() manually to
   update the list, see \ref events_and_pnp_concepts for details
 ...
 */
  ...
  */
  ...
  /**
      ...
      The GetPnPEvent method is used to retrieve plug and play events from the
     camera info list. Those events notify you that one ore more camera has been
     connected or disconnected from the system. If an event is waiting to be
     retrieved, it will return immediately, if no event is available at the time
     the method is called, it will wait for the timeout specified with the
  ...
  */
  ...
  /**
      \brief      Enable event callback to get notified of changes of the
     CamInfoList \param[in]  callback The EventCallback method of the given
     object will be called for every event data. \return     The CamInfoList
     object
  */
