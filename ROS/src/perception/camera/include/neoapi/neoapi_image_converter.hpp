...
/**
    ...
    This class provides methods to configure the Color Matrix for color cameras.
   The Color Matrix can be used to achieve ideal color representation of an
   image during de-bayering. After setting suitable values for the color matrix
   elements, you can pass the instance to a NeoAPI::ConverterSettings instance
   which can than be used with the NeoAPI::Image::Convert method to apply the
   settings to an image.
    ...
    ColorMatrix matrix;                                // Create a ColorMatrix
   object ConverterSettings settings;                        // Create a
   ConverterSettings object matrix.SetValue(ColorMatrix::Gain::Gain00, -0.3); //
   Set a single value settings.SetColorTransformationMatrix(matrix);     // Set
   the ColorMatrix to ConverterSettings \endcode
*/
...
public:
  ...
  */
  enum class Gain {
    Gain00 =
        0,  //!< The red contribution to the red pixels (multiplicative factor)
    Gain01, //!< The green contribution to the red pixels (multiplicative
            //!< factor)
    Gain02, //!< The blue contribution to the red pixels (multiplicative factor)
    Gain10, //!< The red contribution to the green pixels (multiplicative
            //!< factor)
    Gain11, //!< The green contribution to the green pixels (multiplicative
            //!< factor)
    Gain12, //!< The blue contribution to the blue pixels (multiplicative
            //!< factor)
    Gain20, //!< The red contribution to the blue pixels (multiplicative factor)
    Gain21, //!< The green contribution to the blue pixels (multiplicative
            //!< factor)
    Gain22, //!< The blue contribution to the blue pixels (multiplicative
            //!< factor)
  };
...
/**
    ...
    You can pass an instance to the NeoAPI::Image::Convert method to apply the
   settings to an image convertion.
    ...
    Image image;                                                          //
   Create a Image object ConverterSettings settings; // Create a
   ConverterSettings object
    settings.SetSharpeningMode(ConverterSettings::Sharpening::Adaptive);  //
   Configure sharpening settings.SetDebayerFormat("Mono8"); // Set destination
   PixelFormat image.Convert(settings); // Convert the image using the provided
   settings \endcode
*/
...
public:
  ...
  */
  enum class Sharpening {
    ...
    Global,   //!< Sharpening in every area of the image, independent of the
              //!< content.
  ...
  */
  ...
  /**
      ...
      \note      The "=" operator will not copy the actual image converter but
     reference it. For parallel conversions you should create one
     ConverterSettings object for each thread. \param[in] object An
     ConverterSettings object \return    The ConverterSettings object
  */
  ...
  /**
     ...
     the format you need to work with the image data. You can use this setting
     to convert an image from one format to another. (e.g. convert BayerRG8 to
     BGR to work with the image data using OpenCV).

      \note    Please see NeoAPI::PixelFormat for a list of possible formats

      \note    Not all input formats can be converted to all output formats. You
     can use the Image::GetAvailablePixelFormats() or the
  ...
  */
  ...
  /**
      ...
      \param[in]  threshold  The SharpeningSensitivityThreshold for the
     converted image. \return    The ConverterSettings object
  */
  virtual ConverterSettings &
  SetSharpeningSensitivityThreshold(uint32_t threshold);
  ...
  */
  virtual ConverterSettings &
  SetColorTransformationMatrix(const ColorMatrix &matrix);
