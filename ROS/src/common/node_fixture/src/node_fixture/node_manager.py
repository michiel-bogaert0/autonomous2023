...
class NodeManager(ManagedNode):
    ...
    def __init__(self, name: str, default_state = NodeManagingStatesEnum.UNCONFIGURED):
        ...
        self.health_pub = rospy.Publisher("/health/diagnostics", DiagnosticArray, queue_size=1)
    ...
    def check_health(self):
        ...
        keyvalues.append(KeyValue(key="Monitored nodes", value=", ".join(self.nodes_to_monitor)))
        ...
        for node in self.nodes_to_monitor:
            ...
            if node not in self.timers.keys():
                ...
                keyvalues.append(KeyValue(key=node, value="No contact. Might still be starting up"))
                
            # Check if node is still alive, if not, set health to error
            if node in self.timers.keys() and rospy.Time.now().to_sec() - self.timers[node] > self.timeout:
                new_health_level = max(new_health_level, DiagnosticStatus.ERROR)
                keyvalues.append(KeyValue(key=node, value="Lost contact. Node not healthy"))
            # This is fine, but check the "state" of the node
                ...
                if health_msg.values[0].value != "active":
                    ...
                    keyvalues.append(KeyValue(key=node, value="Node not 'active' (yet)"))
        ...
        for node in self.nodes_to_monitor:
            if node in self.health_msgs and self.health_msgs[node].level > new_health_level:
                new_health_level = self.health_msgs[node].level
            
            # Also list ALL monitored nodes warnings and errors to keyvalues
            if node in self.health_msgs and self.health_msgs[node].level > DiagnosticStatus.OK:
                keyvalues.append(KeyValue(key=node, value=self.health_msgs[node].message))
        ...
        if self.health.level != DiagnosticStatus.OK and not self.unhealty_status_self_inflicted:
            new_health_level = max(new_health_level, self.health.level)

        if new_health_level == DiagnosticStatus.OK:
            self.set_health(level=new_health_level, message="All monitored nodes are healthy", values=keyvalues, publish=publish)
            self.unhealty_status_self_inflicted = False
        else:
            message = "There is an issue with at least one monitored node" if self.health.level == DiagnosticStatus.OK else self.health.message
            self.set_health(level=new_health_level, message=message, values=keyvalues, publish=publish)
            self.unhealty_status_self_inflicted = self.health.level != DiagnosticStatus.OK
    ...
    def configure_nodes(self):
        ...
        try:            
            ...
            nodes_to_configure = [item for state in node_management_param.values() for item in state]
            nodes_to_configure = list(set(nodes_to_configure))
                    
            for node in nodes_to_configure:
                configure_node(node)
        
            self.set_health(level=DiagnosticStatus.OK, message=f"Configured nodes: {nodes_to_configure}")
        
            return True
            
        except BaseException as e:
            
            self.set_health(level=DiagnosticStatus.ERROR, message=f"Could not configure nodes. Error: {e}")
    ...
    def activate_nodes(self, new_state, old_state = None):
        ...
        try:
            ...
            node_management_param = self.get_node_management_params(new_state, old_state)

            # First activate nodes (that are not already active)       
            for node in node_management_param[new_state] + node_management_param['always']:
                if old_state is None or node not in node_management_param[old_state] + node_management_param['always'] or (node in node_management_param['always'] and node not in self.nodes_to_monitor):
            ...
            for node in node_management_param[old_state]:
                if node not in node_management_param[new_state] + node_management_param['always']:
                    self.nodes_to_monitor.remove(node)
                    self.timers.pop(node, None)
                    self.health_msgs.pop(node, None)
                    set_state_inactive(node)
                    
            self.set_health(level=DiagnosticStatus.OK, message=f"Activated nodes for {new_state}")
    
        except BaseException as e:
            self.set_health(level=DiagnosticStatus.ERROR, message=f"Could not activate nodes, error: {e}")
