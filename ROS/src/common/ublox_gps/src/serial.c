...
enum SerialStopbits { SPASTOPBITS_1 = 1, SPASTOPBITS_2 = 2 };
enum SerialParity {
  SPAPARITY_NONE = 'N',
  SPAPARITY_ODD = 'O',
  SPAPARITY_EVEN = 'E'
...
static const char *
SerialInit(struct serial *sn, const char *Device, enum SerialBaud Baud,
           enum SerialStopbits StopBits, enum SerialProtocol Protocol,
           enum SerialParity Parity, enum SerialDatabits DataBits,
           int dowrite
#ifdef __GNUC__
...
static const char *SerialInit(struct serial *sn, const char *Device,
  ...
  if ((sn->Stream =
           CreateFile(mydevice[0] ? mydevice : Device,
                      dowrite ? GENERIC_WRITE | GENERIC_READ : GENERIC_READ, 0,
  ...
  snprintf(str, sizeof(str),
           "baud=%d parity=%c data=%d stop=%d xon=%s octs=%s rts=%s", Baud,
