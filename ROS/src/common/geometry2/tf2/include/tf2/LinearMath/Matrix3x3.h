...
public:
  ...
  */
   ...
   */
  ROS_DEPRECATED void setEulerZYX(const tf2Scalar &yaw, const tf2Scalar &pitch,
   ...
   * are possible values*/
  ROS_DEPRECATED void getEulerZYX(tf2Scalar &yaw, tf2Scalar &pitch,
                                  tf2Scalar &roll,
  ...
  void diagonalize(Matrix3x3 &rot, tf2Scalar threshold, int maxSteps) {
    ...
    for (int step = maxSteps; step > 0; step--) {
      ...
      tf2Scalar t = threshold * (tf2Fabs(m_el[0][0]) + tf2Fabs(m_el[1][1]) +
                                 tf2Fabs(m_el[2][2]));
      if (max <= t) {
...
Matrix3x3::serialize(struct Matrix3x3Data &dataOut) const {
  for (int i = 0; i < 3; i++)
    m_el[i].serialize(dataOut.m_el[i]);
}

TF2SIMD_FORCE_INLINE void
Matrix3x3::serializeFloat(struct Matrix3x3FloatData &dataOut) const {
  for (int i = 0; i < 3; i++)
    m_el[i].serializeFloat(dataOut.m_el[i]);
}

TF2SIMD_FORCE_INLINE void
Matrix3x3::deSerialize(const struct Matrix3x3Data &dataIn) {
  for (int i = 0; i < 3; i++)
    m_el[i].deSerialize(dataIn.m_el[i]);
}

TF2SIMD_FORCE_INLINE void
Matrix3x3::deSerializeFloat(const struct Matrix3x3FloatData &dataIn) {
  for (int i = 0; i < 3; i++)
    m_el[i].deSerializeFloat(dataIn.m_el[i]);
}

TF2SIMD_FORCE_INLINE void
Matrix3x3::deSerializeDouble(const struct Matrix3x3DoubleData &dataIn) {
  for (int i = 0; i < 3; i++)
