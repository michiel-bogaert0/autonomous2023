...
public:
  ...
  TF2SIMD_FORCE_INLINE void serializeDouble(struct Vector3DoubleData & dataOut)
      const;
...
Vector3::serializeFloat(struct Vector3FloatData &dataOut) const {
...
TF2SIMD_FORCE_INLINE void
Vector3::deSerializeFloat(const struct Vector3FloatData &dataIn) {
  for (int i = 0; i < 4; i++)
    m_floats[i] = tf2Scalar(dataIn.m_floats[i]);
}

TF2SIMD_FORCE_INLINE void
Vector3::serializeDouble(struct Vector3DoubleData &dataOut) const {
  /// could also do a memcpy, check if it is worth it
  for (int i = 0; i < 4; i++)
...
TF2SIMD_FORCE_INLINE void
Vector3::deSerializeDouble(const struct Vector3DoubleData &dataIn) {
  for (int i = 0; i < 4; i++)
    m_floats[i] = tf2Scalar(dataIn.m_floats[i]);
}

TF2SIMD_FORCE_INLINE void
Vector3::serialize(struct Vector3Data &dataOut) const {
  /// could also do a memcpy, check if it is worth it
  for (int i = 0; i < 4; i++)
...
TF2SIMD_FORCE_INLINE void
Vector3::deSerialize(const struct Vector3Data &dataIn) {
  for (int i = 0; i < 4; i++)
