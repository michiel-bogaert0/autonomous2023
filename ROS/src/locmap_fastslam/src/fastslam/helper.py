...
class Helpers:
    ...
    def calculate_pose_jacobian(landmark, pose):
        ...
        # One jacobian has dimensions of 2x2
        # See course for more information
        # [[dr / dpx, dr / dpy, db / dptheta],
        #  [db / dpx, db / dpy, db / dptheta]]
        H = (
            np.array(
                [
                    [-dx, -dy, 0],
                    [dy / expected_range, -1 * dx / expected_range, -1],
                ]
            )
            / expected_range
        )
    ...
    @staticmethod
    def calculate_full_jacobian(landmark, pose):
        """
        Efficient method to calculate full jacobian
        """
        H = np.zeros((2, 5))
    ...
    def calculate_observation_jacobians(observations, pose):
        """
        Same as calculate_pose_jacobian but for multiple observations
        """
