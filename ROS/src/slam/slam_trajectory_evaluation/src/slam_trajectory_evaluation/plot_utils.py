...
def boxplot_compare(ax, xlabels, data, data_labels, data_colors, legend=True):
    n_data = len(data)
    ...
    for idx, d in enumerate(data):
        ...
        positions = [pos - 0.5 + 1.5 * w + idx * w for pos in np.arange(n_xlabel)]
...
def plot_error_n_dim(ax, distances, errors, results_dir, colors=None, labels=None):
    ...
    for i in range(len(colors)):
        ax.plot(distances, errors[:, i], colors[i] + "-", label=labels[i])
