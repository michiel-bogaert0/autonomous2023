...
class TrackGenerator:
    ...
    @staticmethod
    def write_to_yaml(file_path, start_cones, l_cones, r_cones, overwrite=False):
        if not overwrite and exists(file_path):
            raise FileExistsError(f"'{file_path}' already exists")
        ...
        # Add blue cones
        for cone in l_cones:
            yaml_track["observations"].append(
                {
