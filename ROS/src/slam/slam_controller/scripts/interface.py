...
class Convert(ROSNode):
    """
    This node connects slam to other nodes when a simple remap is not enough.
    """

    def __init__(self):

        super().__init__("slam_external_interface")
        ...
        rospy.loginfo("slam Interface started!")
    ...
    def convertGTOdometry(self, msg: Odometry):
        """
        Converts a generic Odometry message to something that is useable in slam.
