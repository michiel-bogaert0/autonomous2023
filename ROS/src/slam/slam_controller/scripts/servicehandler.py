...
class ServiceHandler:
    """
    This class handles the LaaS (launch as a service) calls fpr slam_controller
    """
    ...
    def __init__(self) -> None:
        ...
        rospy.init_node("slam_controller_servicehandler")

        roslaunch.configure_logging(ServiceHandler.launch_uuid)

        # Setup services
        rospy.Service("slam_controller/Launch", Launch, ServiceHandler.handle_launch)
        rospy.Service("slam_controller/Stop", Stop, ServiceHandler.handle_stop)

        rospy.loginfo("slam Controller Servicehandler is ready to serve!")
    ...
    def handle_launch(req: LaunchRequest):
        """
        Handles the Launch service of slam_controller
        ...
        cli_args = ["slam_controller", filename]
