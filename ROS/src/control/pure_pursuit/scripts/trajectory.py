...
class Trajectory:
    ...
    def set_path(self, points, current_position):
        ...
        self.eucl_dist = np.sqrt( (self.path[:, 0] -  current_position[0]) ** 2 + (self.path[:, 1] - current_position[1]) ** 2)
    ...
    def calculate_target_point(self, minimal_distance, current_position):
        ...
        while not found:
            ...
            distance = (current_position[0] - self.path[self.closest_index][0]) ** 2 + (current_position[1] - self.path[self.closest_index][1]) ** 2
            if distance > minimal_distance ** 2:
                return self.path[self.closest_index][0], self.path[self.closest_index][1], True 
