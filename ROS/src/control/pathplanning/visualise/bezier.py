...
def make_bezier(points: List[QtC.QPointF], is_closed: bool) -> List[BezierPoint]:
    bezier = []
    if len(points) > 2:
        for i, m in enumerate(points[::STEP]):
            if STEP * i == len(points) - 1:
                next = points[0] * is_closed + m * (not is_closed)
            else:
                next = points[STEP * i + 1]

            if i == 0:
                prev = points[-1] * is_closed + m * (not is_closed)
            else:
                    ...
                    if index + removed > 0 or i == 0:
                        ...
                        for yc in yellow_cones[: i + 1]:
                            ...
                            if dis < mindis:
                                mindis = dis
                                closest = yc
                        middlePoints.append((el + closest) / 2)
                removed = j
                nearest_blue = blue_cone
        if nearest_blue:
            middlePoints.append((yellow_cone + nearest_blue) / 2)
...
def get_bezier_curve_iterator(
    bezierPoints: List[BezierPoint], is_closed: bool
    ...
    if is_closed:
        ...
        return zip(bezierPoints[:-1], shifted_by_one[:-1])
