...
class Triangulator:
    ...
    def get_center_points(
        ...
        var_filter = np.median(variances) * self.triangulation_var_threshold
        ...
        df = pd.DataFrame(flattened_center_points)
        centerline = df[df.duplicated()].to_numpy()

        return center_points, centerline
