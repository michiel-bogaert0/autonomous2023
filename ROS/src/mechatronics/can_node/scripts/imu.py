...
class ImuConverter:
    ...
    def handle_imu_msg(self, msg : can.Message, is_front: bool) -> None:
        ...
        latency = msg.buf[7]
        ts_corrected = msg.timestamp - 0.0025 if latency == 5 else msg.timestamp # 2.5 ms latency according to the datasheet
        imu_msg.header.stamp = ts_corrected
        imu_msg.header.frame_id = self.imu_front_frame if is_front else self.imu_back_frame
        ...
        if cmd_id == 0xF029:
            ...
            imu_msg.orientation = tf_conversions.transformations.quaternion_from_euler(roll, pitch, 0)
            imu_msg.orientation_covariance = np.diag([1, 1, 1]).reshape((1, 9)).tolist()[0] # TODO change
            ...
            imu_msg.angular_velocity_covariance = np.diag([1, 1, 1]).reshape((1, 9)).tolist()[0] # TODO change
