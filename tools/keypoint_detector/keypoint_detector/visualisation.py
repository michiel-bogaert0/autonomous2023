...
from keypoint_detector.data import create_heatmaps, retrieve_data, read_img, retrieve_label_file, ConeDataset, \
    KeyPointDataset
import cv2
import numpy as np
...
def plotted_img(plt_img, plt_label, color=(0, 255, 0), thickness=1):
    for (px, py), (nx, ny) in zip(plt_label[:-1], plt_label[1:]):
        plt_img = cv2.arrowedLine(plt_img, (int(px), int(py)), (int(nx), int(ny)), color, thickness)
...
def save_test_sample(img, pt, intered_pt, color=(0, 255, 0), thickness=1):
    ...
    for (px, py), (nx, ny) in zip(pt, intered_pt):
        img = cv2.arrowedLine(img, (int(px), int(py)), (int(nx), int(ny)), color, thickness)
...
for imgs, hms, pts in test_dl:
    ...
    save_test_batch(imgs.detach().cpu().numpy(), pts.detach().cpu().numpy(), infered_pts.detach().cpu().numpy())